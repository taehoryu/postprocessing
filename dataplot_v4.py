...
modelname=['hydro+gravity',\
          r'$N_{\mathrm{vol}}  =100$',\
          r'$A_{\mathrm{vol}}/y=10^{-3}$',\
          r'$\sigma_{\mathrm{vol}}/y=5\times10^{-2}$']
...
#box=1, no box=0
fluxlabel=['$v_{\mathrm{shear}}=1{\mathrm{km}\mathrm{s}^{-1}}$','$v_{\mathrm{shear}}=10{\mathrm{km}\mathrm{s}^{-1}}$','$v_{\mathrm{shear}}=10^{2}{\mathrm{km}\mathrm{s}^{-1}}$']
#linetype=['-','--','-.',':','--','-.',':']
...
for i in profilefilelist:
  ...
  if number_plt==1:
    ...
    if Dimension==1:
      ...
      number_cell_height=number_cell_width
    elif Dimension==2:
...
for plot_type_index in plot_type :
  ...
  if plot_type_index==2:
    ...
    jj=0
    j=0
    color_red=get_color_red(numberoflines+2)
    for i in profilefilelist:
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        if choose_label==0:
          ...
          alabel=fluxlabel[j]
        plt.plot(average_press_in_x/1e6+0.00*float(j),average_vx_in_x/1e5,label=alabel, color=acolor)
              #,label=fluxlabel[j],linestyle=linetype[j], linewidth=linethickness[j], color=linec[j])
        if Dimension ==2:
          plt.fill_between(average_press_in_x/1e6+0.00*float(j),max_vx_in_x/1e5,min_vx_in_x/1e5,alpha=0.2, color=acolor)
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        if choose_label==0:
          ...
          alabel=fluxlabel[j]
        plt.plot(average_press_in_x/1e6+0.00*float(j),average_vy_in_x/1e5,label=alabel, color=acolor)
              #,label=fluxlabel[j],linestyle=linetype[j], linewidth=linethickness[j], color=linec[j])
        if Dimension ==2:
          plt.fill_between(average_press_in_x/1e6+0.00*float(j),max_vy_in_x/1e5,min_vy_in_x/1e5,alpha=0.2, color=acolor)
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        if choose_label==0:
          ...
          alabel=fluxlabel[j]
        plt.plot(average_press_in_x/1e6+0.00*float(j),average_KE_in_x,label=alabel, color=acolor)#
              #,label=fluxlabel[j],linestyle=linetype[j], linewidth=linethickness[j], color=linec[j])
        if Dimension ==2:
  ...
  if plot_type_index==11 and Dimension==2:
    ...
    color_red=get_color_red(numberoflines+2)
    Favre_filelist = sorted(glob.glob('faverage*'), key=lambda name: int(name[-5:]))
    for i in Favre_filelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        if choose_label==0:
          ...
          alabel=fluxlabel[j]
        plt.plot(Faverage_pressure/1e6+0.00*float(j),KE1,label=alabel, color=acolor)
        j=j+1
              #,label=fluxlabel[j],linestyle=linetype[j], linewidth=linethickness[j], color=linec[j])
              #if Dimension ==2:
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        if choose_label==0:
          ...
          alabel=fluxlabel[j]
        plt.plot(x_coord/1e5-0.0*float(j),average_KE_in_x,label=alabel, color=acolor)
        if Dimension ==2:
          plt.fill_between(x_coord/1e5,max_KE_in_x,min_KE_in_x,alpha=0.2, color=acolor)
        
        
        plt.legend(loc=2)#,prop={'size':fontsize1})
        plt.ylabel(r'$sKE\/[\mathrm{cm}^{2}\mathrm{s}^{-2}]$')
    ...
    for i in Favre_filelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        if choose_label==0:
          ...
          alabel=fluxlabel[j]
        plt.plot(height/1e5-0.0*float(j),KE1,label=alabel, color=acolor)
        j=j+1
              #,label=fluxlabel[j],linestyle=linetype[j], linewidth=linethickness[j], color=linec[j])
              #if Dimension ==2:
              # plt.fill_between(Faverage_pressure/1e6,KE1+rms_KE,KE1-rms_KE,alpha=0.2, color=acolor)
  ...
  if plot_type_index==18:
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        plt.ylabel(r'total $ sKE at $h<h$ ($P=$1mbar)\/[\mathrm{cm}^{2}\mathrm{s}^{-2}]$')
        plt.xlabel(r'$h\/[\mathrm{km}]$')
#        plt.xlim(1e-3,P_range[1]*1.1/1e6)
        plt.xlim(0,1.3e4)
        plt.ylim(1e-8,1e15)
  ...
  if plot_type_index==19 and Dimension==2:
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
       ...
       #   plt.fill_between(average_press_in_x/1e6,max_KE_in_x,min_KE_in_x,alpha=0.2, color=acolor)
        plt.legend(loc=3)#,prop={'size':fontsize1})
        plt.ylabel(r'total $\tilde{k} at $h<h$ ($P=$1mbar)\/[\mathrm{cm}^{2}\mathrm{s}^{-2}]$')
        plt.xlabel(r'$h\/[\mathrm{km}]$')
        #plt.xlim(1e-3,P_range[1]*1.1/1e6)
        plt.xlim(0,1.3e4)
        plt.ylim(1e-8,1e15)
        ...
        while (text_num<=3):
          if textbox[text_num]==0:
            ...
            plt.figtext(textposition_RB[0],textposition_RB[1]-(0.05*(text_num-1)+0.05),modelname[text_num],color=color[text_num], fontsize=13,bbox=bbox_props[text_num])
          text_num=text_num+1
        plt.title(r'total average turbulent kinetic energy at $h<h$ ($P=$1mbar) $\tilde{k}$ vs height $h$')
...
#      plt.xlim(1e-3,P_range[1]*1.1/1e6)
        ...
        while (text_num<=3):
          if textbox[text_num]==0:
            ...
            plt.figtext(textposition_RB[0],textposition_RB[1]-(0.05*(text_num-1)+0.05),modelname[text_num],color=color[text_num], fontsize=13,bbox=bbox_props[text_num])
          text_num=text_num+1
        plt.title(r'cumulative average turbulent kinetic energy at $h<h$ ($P=$1mbar) $\tilde{k}$ vs height $h$')
  ...
  if plot_type_index==12:
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
        ...
        plt.legend(loc=4)#,prop={'size':fontsize1})
        plt.ylabel(r'total $ sKE (P>1mbar)\/[\mathrm{cm}^{2}\mathrm{s}^{-2}]$')
        plt.xlabel(r'$P\/[\mathrm{bar}]$')
#        plt.xlim(1e-3,P_range[1]*1.1/1e6)
        plt.xlim(0,0.1)
        plt.ylim(1e-8,1e15)
        ...
        while (text_num<=3):
          if textbox[text_num]==0:
            plt.figtext(textposition_LB[0]+0.2,textposition_RB[1]-(0.05*(text_num-1)+0.05),modelname[text_num],color=color[text_num], fontsize=13)
          elif textbox[text_num]==1:
            plt.figtext(textposition_LB[0]+0.2,textposition_RB[1]-(0.05*(text_num-1)+0.05),modelname[text_num],color=color[text_num], fontsize=13,bbox=bbox_props[text_num])
          text_num=text_num+1
        plt.title(r'total specific kinetic energy (P>1mbar) $sKE$ vs height $h$')
  ...
  if plot_type_index==13 and Dimension==2:
    ...
    for i in profilefilelist:
      ...
      if (time>=float(jj)*time_difference_x_T and time<=max_time) or time==max_time:
       ...
       #   plt.fill_between(average_press_in_x/1e6,max_KE_in_x,min_KE_in_x,alpha=0.2, color=acolor)
        plt.legend(loc=4)#,prop={'size':fontsize1})
        plt.ylabel(r'total $\tilde{k} (P>1mbar)\/[\mathrm{cm}^{2}\mathrm{s}^{-2}]$')
        plt.xlabel(r'$P\/[\mathrm{bar}]$')
        #plt.xlim(1e-3,P_range[1]*1.1/1e6)
        plt.xlim(0,0.1)
        plt.ylim(1e-8,1e15)
...
#      plt.xlim(1e-3,P_range[1]*1.1/1e6)
        ...
        while (text_num<=3):
          if textbox[text_num]==0:
            ...
            plt.figtext(textposition_LB[0]+0.2,textposition_RB[1]-(0.05*(text_num-1)+0.05),modelname[text_num],color=color[text_num], fontsize=13,bbox=bbox_props[text_num])
          text_num=text_num+1
        plt.title(r'cumulative average turbulent kinetic energy (P>1mbar) $\tilde{k}$ vs height $h$')
